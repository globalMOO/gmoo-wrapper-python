...
class ApplicationOperations:
    ...
    This class contains methods for using trained inverse models,
    ...
    def assign_objectives_target(self, objectives_target: List[float], 
        ...
        if objective_types is not None and len(objectives_target) != len(objective_types):
            raise ValueError(f"Array length mismatch: {len(objectives_target)} targets vs {len(objective_types)} types")
        
        # Validate that targets array length matches expected number of objectives
        if len(objectives_target) != self.api.nObjs.value:
            raise ValueError(f"Number of targets ({len(objectives_target)}) does not match expected number of objectives ({self.api.nObjs.value})")
        ...
        if objective_types is None:
            ...
            # Validate objective types length matches number of objectives
            if len(objective_types) != self.api.nObjs.value:
                raise ValueError(f"Number of objective types ({len(objective_types)}) does not match expected number of objectives ({self.api.nObjs.value})")
    ...
    def calculate_initial_solution(self, override_init: Union[bool, List[float]], 
                                  iBias: int = 0, iPipe: int = 1) -> ctypes.Array:
        """
        ...
        Args:
            ...
            iPipe: The pipe identifier (default 1). Note: This was incorrectly named iGen in earlier versions.
            
        Returns:
            ctypes.Array: C-compatible array of the initial variable values.
        """
        self.api.iBias = ctypes.c_int(iBias)
        self.api.iPipe = ctypes.c_int(iPipe)
        ...
        self.api.VSMEappLoadVariableValues(
            ...
            ctypes.byref(self.api.iPipe), 
            ctypes.byref(self.api.d_varj_values)
        )
                           
        return self.api.d_varj_values

    def load_variable_values(self, variable_arr: List[float], iBias: int = 0, iPipe: int = 1) -> None:
        """
    ...
    def load_outcome_values(self, output_arr: List[float], iBias: int = 0, iPipe: int = 1) -> None:
        """
    ...
    def fetch_variables_for_next_iteration(self, iBias: int = 0, iPipe: int = 1) -> np.ndarray:
        """
    ...
    def poke_learned_case(self, nVars: int, nOuts: int, iCase: int, iPipe: int = 1) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        Args:
            ...
            iPipe: Pipe identifier (default 1).
    ...
    def poke_pipe_learned_cases(self, pipe: int) -> ctypes.c_int:
        """
        Retrieve the number of learned cases for a specific pipe.
        
        Args:
            pipe: Pipe number to query
            
        Returns:
            ctypes.c_int: Number of learned cases in the specified pipe
        """
        nVars = ctypes.c_int(0)
        nOuts = ctypes.c_int(0)
        getPipe = ctypes.c_int(pipe)
        nPipes = ctypes.c_int(0)
        nCats = ctypes.c_int(0)
        nLrns = ctypes.c_int(0)
        n7 = ctypes.c_int(0)
        n8 = ctypes.c_int(0)
        ...
        self.api.VSMEappPokeDimensions10(
            ctypes.byref(nVars),   # NI1
            ctypes.byref(nOuts),   # NI2
            ctypes.byref(getPipe), # NI3 - used as pipe selector in some contexts
            ctypes.byref(nPipes),  # NI4
            ctypes.byref(nCats),   # NI5
            ctypes.byref(nLrns),   # NI6
            ctypes.byref(n7),
            ctypes.byref(n8),
            ctypes.byref(n9),
            ctypes.byref(n10)
    ...
    def load_learned_case(self, nVars: int, nOuts: int, dVarValues: List[float], 
                         dOutValues: List[float], iPipe: int = 1) -> None:
        """
    ...
    def load_variable_values_for_pipe(self, variable_arr: List[float], iPipe: int = 1, iBias: int = 0) -> None:
        """
        Load specific variable values into the VSME model for a specific pipe.
        
        Args:
            variable_arr: The array of variable values to load.
            iPipe: The pipe identifier (1-based, default 1).
            iBias: The bias index (default 0, no bias).
        """
        self.api.iBias = ctypes.c_int(iBias)
        self.api.iPipe = ctypes.c_int(iPipe)
        ...
        # Load the values into the DLL
        # Note: The DLL expects iBias in the second position and iPipe in the third
        self.api.VSMEappLoadVariableValues(
            ctypes.byref(self.api.nVars), 
            ctypes.byref(self.api.iBias), 
            ctypes.byref(self.api.iPipe), 
    ...
    def load_outcome_values_for_pipe(self, output_arr: List[float], iPipe: int = 1, iBias: int = 0) -> None:
        """
        Load outcome values into the DLL for optimization for a specific pipe.
    ...
    def fetch_variables_for_next_iteration_pipe(self, iPipe: int = 1, iBias: int = 0) -> np.ndarray:
        """
        Retrieve the suggested variable values for the next optimization iteration for a specific pipe.
    ...
    def perform_inverse_iteration(self, target_outputs: List[float],
                                 current_inputs: Union[List[float], List[List[float]]],
                                 current_outputs: Union[List[float], List[List[float]]],
                                 ...
                                 objective_uncertainty_plus: Optional[List[float]] = None) -> Union[Tuple[np.ndarray, float, float], Tuple[List[np.ndarray], List[float], List[float]]]:
        """
        ...
        for the next iteration. Can handle single pipe or multiple pipes.
        
        Args:
            target_outputs: Target output values to match
            current_inputs: Current input values (single pipe: List[float], multi-pipe: List[List[float]])
            current_outputs: Current output values (single pipe: List[float], multi-pipe: List[List[float]])
        ...
        Returns:
            For single pipe - Tuple containing:
            ...
            For multi-pipe - Tuple containing:
                List[np.ndarray]: Suggested input values for each pipe
                List[float]: L1 norms for each pipe
                List[float]: L2 norms for each pipe
        """
        # Import validate_nan from helpers
        from .helpers import validate_nan
        
        # Check if we're dealing with multiple pipes
        # Multi-pipe: current_inputs is a list of arrays/lists
        # Single-pipe: current_inputs is a single array/list
        is_multi_pipe = (isinstance(current_inputs, list) and 
                        len(current_inputs) > 0 and 
                        (isinstance(current_inputs[0], (list, np.ndarray))))
        
        if is_multi_pipe:
            # Multi-pipe case
            num_pipes = len(current_inputs)
            
            # Validate all inputs for NaN values
            for i, (inputs, outputs) in enumerate(zip(current_inputs, current_outputs)):
                validate_nan(inputs, f"current_inputs[{i}]")
                validate_nan(outputs, f"current_outputs[{i}]")
            validate_nan(target_outputs, "target_outputs")
            
            # Set objective targets and types (same for all pipes)
            ...
            for pipe_idx in range(num_pipes):
                # Calculate error metrics for this pipe
                l2norm = norm(np.array(current_outputs[pipe_idx]) - np.array(target_outputs), ord=2)
                
                # Calculate L1 norm (only for active objectives if specified)
                if objective_status is not None:
                    l1norm = norm(
                        np.array(current_outputs[pipe_idx]) * np.array(objective_status) - 
                        np.array(target_outputs) * np.array(objective_status), 
                        ord=1
                    )
                else:
                    l1norm = norm(np.array(current_outputs[pipe_idx]) - np.array(target_outputs), ord=1)
                
                l1_norms.append(l1norm)
                l2_norms.append(l2norm)
                
                # Load current variables and outcomes for this pipe
                # Note: Pipes are 1-indexed in the DLL
                self.load_variable_values_for_pipe(current_inputs[pipe_idx], iPipe=pipe_idx + 1, iBias=0)
                self.load_outcome_values_for_pipe(current_outputs[pipe_idx], iPipe=pipe_idx + 1, iBias=0)
            
            # Run optimization iteration for all pipes at once
            self.run_vsme_app(1)
            
            # Get suggested variables for next iteration for each pipe
            for pipe_idx in range(num_pipes):
                next_inputs = self.fetch_variables_for_next_iteration_pipe(iPipe=pipe_idx + 1, iBias=0)
                next_inputs_list.append(next_inputs)
            
            return next_inputs_list, l1_norms, l2_norms
        
        else:
            # Single pipe case (original implementation)
            validate_nan(current_inputs, "current_inputs")
            validate_nan(current_outputs, "current_outputs")
            validate_nan(target_outputs, "target_outputs")
            ...
            self.load_variable_values(current_inputs, iBias=0, iPipe=1)
            self.load_outcome_values(current_outputs, iBias=0, iPipe=1)

            # Run optimization iteration
            self.run_vsme_app(1)  # We're just running a single iteration

            # Get suggested variables for next iteration
            next_inputs = self.fetch_variables_for_next_iteration(iBias=0, iPipe=1)
