...
creating and training surrogate models using the GMOO DLL. This includes methods
...
class DevelopmentOperations:
    ...
    This class contains methods for training surrogate models, including
    ...
    def load_vsme_name(self, compatibility_mode: bool = False) -> None:
        ...
        if compatibility_mode:
            ...
            from gmoo_sdk.helpers import c_string_compatibility
    ...
    def init_backup_file(self, override_name: Optional[str] = None) -> None:
        ...
        self.api.cVSMEname = fortran_hollerith_string(pyPath, pad_len=256)
    ...
    def read_backup_file(self, override_name: Optional[str] = None) -> None:
        ...
        self.api.cVSMEname = fortran_hollerith_string(path, pad_len=256)
    ...
    def design_agents(self) -> None:
        ...
        to build the surrogate model for the problem space.
        """
    ...
    def design_cases(self) -> None:
        ...
        to explore the problem space and build the surrogate model.
        """
    ...
    def export_vsme(self, vsme_output_filename: Optional[str] = None) -> str:
        ...
        if vsme_output_filename is None:
            vsme_output_filename = f"{self.api.vsme_input_filename}.gmoo"
        ...
        return vsme_output_filename

    def develop_vsme(self) -> None:
        """
        Develop the VSME surrogate model.
        
        This method builds the surrogate model based on the loaded development cases.
    ...
    def poke_category_label(self, var_id: int, cat_id: int) -> str:
        """
        Retrieve a specific category label from a categorical variable.
        
        Args:
            var_id: The 1-based index of the categorical variable.
            cat_id: The 1-based index of the category within that variable.
            
        Returns:
            str: The category label stored in the DLL.
        """
        logger.info(f"Retrieving category label for variable {var_id}, category {cat_id}")
        
        iVar = ctypes.c_int(var_id)
        iCat = ctypes.c_int(cat_id)
        
        # Prepare a buffer to receive the category label
        outCatLabel = fortran_hollerith_string("X", pad_len=32)

        # Retrieve the category label from the DLL
        self.api.VSMEdevPokeCategoryLabel(
            ctypes.byref(iVar),
            ctypes.byref(iCat),
            ctypes.byref(outCatLabel)
        )
                           
        return outCatLabel.value.decode('ascii').rstrip('\0')
