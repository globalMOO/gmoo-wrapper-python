...
These functions orchestrate the process of training surrogate models and performing
...
from gmoo_sdk.helpers import fortran_hollerith_string, validate_nan
...
def pyVSMEDevelopment(
    ...
    This function manages the complete workflow for training a surrogate model:
...
def pyVSMEInverse(
    ...
    multiple convergence criteria, constraints, and restart strategies.
    
    Args:
        model: The GMOOAPI object for this problem.
        objectives_target: Target output values to be matched.
        convergence: L2 norm convergence criterion for successful optimization.
        unbias_vector: Optional multiplier for de-biasing the input variables.
    ...
    for oo in range(0, outer_loops):
        ...
        for kk in range(0, max_inverse_iterations):
            ...
            if l2norm < convergence:
                logger.info(f"Convergence achieved in inverse loop, l2={l2norm} on iteration "
...
def pyVSMEBias(
    ...
    Returns:
        Tuple containing:
            ...
            bool: Success flag (False if convergence failed).
            str: Message explaining the result.
    ...
    for ll in range(1, n_bias + 1):
        ...
        for kk in range(1, max_inverse_iterations + 1):
            ...
            # Check for errors or convergence
            if model.last_status > 0: 
                ...
                logger.info(f"Convergence (code {model.last_status}) on outer loop iteration {ll}, "
...
def random_search(
    ...
    if csv_mode:
        ...
        # Read outcomes from the CSV file produced by the external program
        output_vectors = model.development.read_outcomes_csv(runs_type="RAND")
